...
```{r vars}
variable_list <- c("female", "age", "native", "loc", "edu",
                    "hhsize", "ch001_", "outside_help", "chronic_mod", 
                   "life_qual", "eurod", "doc_vis", "hosp_stay", 
                   "nursing" , "iadlza", "adla" , "maxgrip", "bmi2", 
                   "ever_smoked", "vig_act", "job_situ", "ends_meet")

binary_list <- c("female",  "native", "outside_help",  
                 "hosp_stay", "nursing" , "ever_smoked")

categorical_list <- c("loc", "edu", "mar_stat", "hhsize", "ch001_", "chronic_mod", 
                     "life_qual", "eurod", "doc_vis", "iadlza", "adla" , "maxgrip", 
                     "bmi2", "vig_act", "job_situ", "ends_meet")
...
boxplot(cogscore ~ native, data = working_data, ylab = "Composite Cognitive score")
...
boxplot(cogscore ~ life_qual, data = working_data, ylab = "Composite Cognitive score")
boxplot(cogscore ~ eurod, data = working_data, ylab = "Composite Cognitive score")
boxplot(cogscore ~ doc_vis, data = working_data, ylab = "Composite Cognitive score")
boxplot(cogscore ~ hosp_stay, data = working_data, ylab = "Composite Cognitive score")
...
boxplot(cogscore ~ job_situ, data = working_data, ylab = "Composite Cognitive score")
boxplot(cogscore ~ ends_meet, data = working_data, ylab = "Composite Cognitive score")
```
...
mod <- cogscore ~ female + age + native + loc + edu +
  hhsize + outside_help + chronic_mod + life_qual + eurod + doc_vis + hosp_stay + nursing +
...
# compare it with a minimal model, take the mean, or just a model with age, 
...
```{r}
mean_mod <- cogscore ~ female 

# Define model
mean_fit <- lm(mean_mod, data = na.omit(training_data))
...
# Add trace=F to not print each step of the step function
mean_better_fit <- step(mean_fit, trace = F)
summary(mean_better_fit)
plot(mean_better_fit)

compute_se(model = mean_fit, test_data = test1)
compute_se(model = mean_fit, test_data = test2)
compute_se(model = mean_fit, test_data = na.omit(test3))
compute_se(model = mean_fit, test_data = na.omit(test4))
```
