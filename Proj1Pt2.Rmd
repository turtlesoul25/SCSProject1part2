...
working_data %>% filter(edu != "Other") %>% 
  ggplot(aes(x = edu, y = cogscore, fill = edu)) +
  geom_boxplot(xlab = "Education level completed", ylab = "Composite Cognitive score", show.legend = F) +
  coord_flip() + stat_boxplot(geom = "errorbar", width = 0.2)

working_data %>% 
  ggplot(aes(x = chronic_mod, y = cogscore, fill = chronic_mod)) +
  geom_boxplot(xlab = "Number of chronic diseases", ylab = "Composite Cognitive score", show.legend = F) +
  coord_flip() + stat_boxplot(geom = "errorbar", width = 0.2)

working_data %>% 
  ggplot(aes(x = eurod, y = cogscore, fill = eurod)) +
  geom_boxplot(xlab = "Depression scale EURO-D", ylab = "Composite Cognitive score", show.legend = F) +
  coord_flip() + stat_boxplot(geom = "errorbar", width = 0.2)

working_data %>% 
  ggplot(aes(x = vig_act, y = cogscore, fill = vig_act)) +
  geom_boxplot(xlab = "Frequency of Vigorous Activities", ylab = "Composite Cognitive score", show.legend = F) +
  coord_flip() + stat_boxplot(geom = "errorbar", width = 0.2) 

working_data %>% 
  ggplot(aes(x = iadlza, y = cogscore, fill = iadlza)) +
  geom_boxplot(xlab = "Difficulties with Instrumental Activities of Daily Living Indices",
               ylab = "Composite Cognitive score", show.legend = F) +
  coord_flip() + stat_boxplot(geom = "errorbar", width = 0.2)

boxplot(cogscore ~ iadlza, data = working_data, xlab = "Composite Cognitive score", horizontal = T)
boxplot(cogscore ~ vig_act, data = working_data, xlab = "Composite Cognitive score", horizontal = T)
# boxplot(cogscore ~ adla, data = working_data, xlab = "Composite Cognitive score", horizontal = T)
# boxplot(cogscore ~ life_quality, data = working_data, xlab = "Composite Cognitive score", horizontal = T)

fitinteract <- lm(cogscore ~ female* age, data = working_data)
interact_plot(fitinteract, pred = age, modx = female, interval = TRUE, line.thickness = 0.3, point.alpha = 1) 
