...
edu_plot <- working_data %>% filter(edu != "Other") %>% 
  ...
  geom_boxplot(show.legend = F) + xlab("Education level completed") + ylab("Composite Cognitive score") + coord_flip() + stat_boxplot(geom = "errorbar", width = 0.2) + theme(axis.text = element_text(size = rel(1.5)))
...
age_female_plot <- interact_plot(fitinteract, pred = age, modx = female, interval = TRUE, line.thickness = 0.3, point.alpha = 1, x.label = "Age", y.label = "Composite Cognitive Score", 
                                 legend.main = "Female")
...
forest_names <- c("female", "age", "native", "loc_Town_rural", "edu_Post_Secondary",
                  "outside_help", "iadlza_High_Difficulty", "vig_act_Rarely", 
                  "age:doc_vis_More_than_10")
forest_plot_df <- fit_df[forest_names, ]
forest_plot_df[, c(1,5)] |> forestplot(labeltext = rownames(forest_plot_df), 
           mean = forest_plot_df$Estimate,
           lower = forest_plot_df$CI_lower, 
           upper = forest_plot_df$CI_upper,
           boxsize = 0.18, xlab = "Coefficient estimate value",
           fn.ci_norm=matrix(c("fpDrawCircleCI"), 
